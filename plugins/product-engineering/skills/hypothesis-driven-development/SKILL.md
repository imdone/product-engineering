...
- Retain the explicitly selected story as `sessionStoryKey` independently from global `active_story`; note-only redirects and global-current changes do not replace it. Before any push reminder, run `imdone status <sessionStoryKey> -f json`; unrelated dirty stories must not trigger a prompt, and the only command offered may be `imdone push <sessionStoryKey>`.
- Event-triggered progress-note activation and detection require the current session to be using this HDD skill and carry `imdone-session-story:v1`. Story tags and `hddProgressNotes:enabled` do not activate ordinary direct-agent sessions. An `imdone ai` context load stays quiet even though it also carries the session-story marker for story retention; its `imdone-story-context:v1` marker takes precedence and does not activate HDD behavior.
- In an activated HDD session, reuse `hddProgressNotes:enabled` from the story's closing metadata comment. When that metadata is missing, ask whether to enable progress notes for the named story. If accepted, persist `hddProgressNotes:enabled` in story metadata; if persistence fails, keep the accepted session enabled, warn that the preference was not saved, and allow a later HDD session to ask again. A decline or unrecognized answer disables capture only for that session and writes no preference.
