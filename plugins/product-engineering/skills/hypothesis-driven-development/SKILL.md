...
- Before asking where to record an enabled progress-note event, recommend the current story first plus at most one best-fit HDD-eligible alternative whose closing metadata comment contains `#HDD-template` or `#HDD-light-template`. Support Jira keys and numeric GitHub issues generically, never replace current or session story state during matching, and allow an explicit issue-key redirect even when that story was not recommended.
