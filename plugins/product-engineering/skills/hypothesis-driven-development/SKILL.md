...
4. Resolve full or lightweight HDD as soon as the story, progress notes, and attachment links are read, before applying or repairing any HDD template and before doing workflow artifact or phase work. Preserve an explicit mode choice from the current conversation; otherwise infer saved mode intent from closing template metadata using `references/session-setup.md` and `scripts/detect_hdd_mode.mjs`. Ask the exact numbered mode prompt only when mode cannot be inferred safely.
5. Handle templates by resolved mode using `references/session-setup.md`: full HDD applies or repairs the full HDD template when needed; lightweight HDD templates are optional and used only when the engineer asks or one is already present. If imdone is unavailable, use the full HDD expected headings for full mode, and keep lightweight HDD sparse around Problem, Current Plan, and the Progress notes attachment.
...
- Before displaying any push reminder in an imdone project, use `imdone agent-config get-config` to determine whether the configured phase or interval makes a reminder due, then apply the natural-stopping-point policy in `references/interaction-contract.md`. Reminder eligibility never creates a new interaction turn. If imdone is unavailable, skip push reminders and record any publish/sync need as an external evidence gate.
- Retain the explicitly selected story as `sessionStoryKey` independently from global `active_story`; note-only redirects and global-current changes do not replace it. At an existing natural stopping point, run `imdone status <sessionStoryKey> -f json` before displaying a due reminder. Unrelated dirty stories and a clean retained story must not trigger a reminder. A reminder tells the developer to run only `imdone push <sessionStoryKey>` when ready; it is not a question and never offers agent execution.
- An explicit request to use this HDD skill enables event-triggered progress-note capture for the resolved session story. The `imdone-session-story:v1` marker from `imdone hdd`, the `imdone-story-context:v1` marker from the `imdone ai` command, and an explicit request to use the `imdone-ai` skill also enable progress-note capture. An explicit opt-out always wins. Without an explicit request or recognized marker, capture stays off. Never ask for consent to enable progress notes.
- Story tags and legacy `hddProgressNotes:enabled` metadata do not activate ordinary direct-agent sessions and are not required by recognized HDD or `imdone ai` sessions. Keep legacy metadata intact for compatibility, but do not use it as an activation gate or persist it as part of session activation.
- In an enabled marked HDD session, `sessionStoryKey` is the authoritative default target for detected progress notes. Confirm that target through the correlated event lifecycle and proceed to `imdone note` without a target-selection question. Support Jira keys and numeric GitHub issues: a developer may explicitly name a locally resolvable key as a note-only redirect; confirm it for that event without replacing current or session story state. If automatic confirmation fails or the session story no longer resolves, fail closed and use the bounded recovery prompt with at most one best-fit HDD-eligible alternative whose closing metadata comment contains `#HDD-template` or `#HDD-light-template`. Do not persist raw context, story bodies, or ranking scores.
- Keep best-fit matching local and bounded. Do not persist raw context, story bodies, or ranking scores; retain only the minimum candidate identity needed for the current session.
...
- After Hypothesis is complete, ask once whether to continue through the remaining HDD workflow without routine per-artifact interruption. Respect the chosen continuation scope until a blocker, missing decision, external evidence gate, major pivot, required approval, or user correction requires stopping.
- When the session-local continuation-through-Plan scope is selected, `references/interaction-contract.md` overrides routine review and reminder timing: defer routine per-artifact review and carry any due phase/progress-note reminder forward to the evaluated Plan checkpoint. Do not defer explicit user-requested pushes, blockers, missing decisions, external evidence gates, major pivots, user corrections, required tool approvals, or failed evaluators.
...
- In `phase_prompt` mode inside an imdone project, completing a phase, first writing a real `attachments/plan.md`, or recording `attachments/progress-notes.md` makes a reminder eligible but does not create a stopping point. Carry the reminder to the next natural stopping point defined in `references/interaction-contract.md`.
- Outside an imdone project, skip push reminders.
