...
- For event-triggered progress notes in a direct agent session, ask once at session start whether capture is enabled and keep the answer session-local; a decline or unrecognized answer leaves capture off for that session. An explicit HDD or story-context launch may enable event-triggered progress notes automatically without another question.
- Retain the explicitly selected session story independently from any repository-global current-story setting. A progress-note redirect, best-fit proposal, or external current-story change must not replace the session story unless the user explicitly selects a different story for the session.
- Before asking where to record an enabled progress-note event, compare the current high-signal context against all locally available backlog stories. Support Jira-style keys and numeric GitHub issues generically, propose the best fit with unnumbered alternatives, and use the session story or repository-global current story only as ranking priors or no-match fallbacks.
- Keep best-fit matching local and bounded. Do not persist raw context, story bodies, or ranking scores; retain only the minimum candidate identity needed for the current session, and never mutate session or repository-global story state merely because another story ranks higher.
